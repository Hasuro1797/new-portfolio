...
import { SiExpress, SiRedux, SiStrapi, SiStyledcomponents } from "react-icons/si";
...
export const ProjectCard = ({
  ...
  const selectIcon = (state: string) => {
    switch (state) {
      case "Active": return <CgMediaLive />;
      case "Disable": return <GiSightDisabled />;
      default: return <AiOutlineDisconnect />
    }
  }
  const techIcons = [
  ...
  return (
    <li className="grid md:grid-cols-2 gap-4">
      <div className="rounded-2xl overflow-clip aspect-auto sm:aspect-[432/244] border border-primary-400/30">
        <img src={image} alt={title} loading="lazy" className="rounded-2xl object-cover object-top md:scale-110 h-auto sm:h-full w-full hover:scale-105 transition duration-500"/>
      </div>
        ...
        <div className="absolute top-2 right-0 md:right-2">
          {
            <Chip
              ...
              label={state}
              styles={`${state === "Active" ? 'bg-secondary': state === "Dead" ? 'bg-tertiary': 'bg-slate-400' }`}
            />
          }
        </div>
        <small className="text-primary-200/70">{type}</small>
        <div className="flex flex-col gap-2">
          <h3 className="text-primary-400 font-semibold text-[24px]">{title}</h3>
          ...
          <p className="text-primary-200/70 text-pretty pt-2">{pathname.substring(1) === 'en'? description.en : description.es }</p>
