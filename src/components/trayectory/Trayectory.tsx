...
export default function Trayectory({
  ...
  const formatDate = (date: string) =>{
    if(date === "Actualmente") return pathname.substring(1) === "en" ? "Currently": date
  ...
  return (
    <section className='flex flex-col gap-10 py-8 mx-auto max-w-[700px] xl:max-w-[900px] xxl:max-w-[1100px]'>
      <h2 className='text-secondary font-bold text-[px]'>
        {title}
      </h2>
            ...
            (
              ...
              >
                ...
                <article className='grid md:grid-cols-5 gap-4 md:gap-10'>
                  <div className={`${item.description === null ? 'md:col-span-5': 'md:col-span-2'} text-primary-200`}>
                    <h3 className='text-secondary text-[20px] font-semibold'>{item.title}</h3>
                    <h4>{item.company}  {item.tyOfOcupation !== null ? " · " + item.tyOfOcupation : ""}</h4>
                    <time className='text-[14px] text-primary-200/70'>{`${formatDate(item.date.since)} - ${formatDate(item.date.until)}`}</time>
                    <div className='text-primary-200/70 text-pretty'>
                      <small>{item.location}</small> {", " }
                      {item.mode !== null && (
                        <small className='text-secondary/80'>{pathname.substring(1) === 'en' ? item.mode.en : item.mode.es }</small>
                      )}
                    </div>
                  </div>
                  {item.description !== null && (
                    <div className='md:col-span-3 text-primary-200/70 text-pretty'>
                      {pathname.substring(1) === 'en' ? item.description.en : item.description.es }
                    </div>
                  )}
                </article>
              </li>
